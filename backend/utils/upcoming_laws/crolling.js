//국가법령정보-최신법령정보-시행예정법령 (과학기술통신부 소관)
...
async function crawlLaws() {
  const baseUrl = 'https://www.law.go.kr/LSW/efLsPop.do';  // URL 변경
  ...
  try {
    while (true) {
      params.pg = page;
      const response = await axios.get(baseUrl, { params });  // 쿼리 파라미터와 함께 GET 요청
